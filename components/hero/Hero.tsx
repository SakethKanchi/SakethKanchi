    ...
    >
      ...
      <div className="relative grid grid-cols-1 gap-3 md:grid-cols-3 md:auto-rows-[minmax(0,1fr)]">
        {/* 1. Identity — largest, spans 2 cols / 2 rows on desktop.
            NOT wrapped in Reveal: this holds the LCP element (the h1), so it
            must paint at full opacity on first frame. Hiding it via autoAlpha
            delays LCP. */}
        <BentoCard
          tilt={false}
          className="flex h-full flex-col justify-between gap-8 p-8 md:col-span-2 md:row-span-2 md:p-10"
        >
        ...
        {/* 2. Stat — accent-soft surface, accent number.
            Hero cards render at full opacity immediately (no Reveal hide) —
            they're above the fold, so hiding them would only delay LCP. */}
        ...
        <BentoCard tilt={false} className="flex flex-wrap items-center gap-2 md:col-span-3">
          {skillPills.map((s) => (
            <span
              key={s}
